...
Page({
  ...
  bindChange(e) {
    ...
    if (e.detail.value[1] === 0 || e.detail.value[1] === 2 || e.detail.value[1] === 4 || e.detail.value[1] === 6 || e.detail.value[1] === 7 || e.detail.value[1] === 9 || e.detail.value[1] === 11) {
      ...
      this.setData({
        days: days,
        begibegindateNewndate: this.data.years[val[0]] + '-' + this.data.months[val[1]] + '-' + this.data.days[val[2]]
      })
    }
  },
  bindChangeEnd(e) {
  ...
  keep: function () {
    ...
    wx.request({
      url: app.config.uploadHost + `/resumes/${app.globalData.resumeId}/trainings/${this.data.trainList.id}`, // 拼接接口地址(前面为公共部分)
      method: 'put',
      header: {
  ...
  onLoad: function (options) {
    this.initValidate()
    let train = JSON.parse(options.train);
    console.log(train)
    this.setData({
      trainList: train,
      institution: train.institution,
      lesson: train.lesson,
      begindate: time.formatDate(train.beginTime),
      enddate: time.formatDate(train.endTime),
    });
  },
